...
| 다중 인증 (MFA) [필수] | TOTP MFA (RFC 6238) - ID/PW + TOTP 복합 인증 필수 |
| 접근 제어 | RBAC 기반 역할별 접근 제어 정책 적용 |
| 암호키 관리 [필수] | RSA 키 페어 생성, 보관, 로테이션, 파기 |
...
│                                                              │
│  ┌──────────────────┐  ┌──────────────────┐                  │
│  │  SSO Server      │  │  SSO Agent       │  │ Admin Console │ │
│  │  (OIDC Provider) │  │  (Servlet Filter)│  │ (관리 UI)     │ │
│  │                  │  │                  │  │               │ │
│  │  - 인증/인가     │  │  - 토큰 검증     │  │ - 사용자 관리 │ │
│  │  - 토큰 관리     │  │  - 세션 연계     │  │ - 감사 조회   │ │
│  │  - 세션 관리     │  │  - 요청 필터링   │  │ - 정책 관리   │ │
...
- **Admin Console**: Next.js 14 기반 웹 관리 인터페이스 (TOE - 관리 보안 기능 제공)
...
| **FIA_UAU.5** | 복합 인증 메커니즘 | TOE는 ID/PW + TOTP MFA 복합 인증을 필수로 지원한다 (RFC 6238). MFA는 TOE 필수 구성요소이다. |
| **FIA_SOS.1** | 비밀 검증 | TOE는 비밀번호 복잡도/길이 정책을 검증한다 |
...
| SFR | 요구사항명 | 설명 |
|-----|-----------|------|
| **FCS_CKM.1** | 암호키 생성 [필수] | TOE는 RSA-2048 이상의 키 페어를 생성한다. 키 관리는 TOE 필수 구성요소이다. |
| **FCS_CKM.2** | 암호키 분배 [필수] | TOE는 JWKS 엔드포인트를 통해 공개키를 안전하게 분배한다 |
| **FCS_CKM.4** | 암호키 파기 [필수] | TOE는 키 로테이션 시 이전 키를 안전하게 파기한다 |
| **FCS_COP.1** | 암호 연산 [필수] | TOE는 RS256(JWT 서명), BCrypt(비밀번호), AES-256-GCM(키 암호화), HMAC-SHA1(TOTP) 연산을 수행한다 |
...
| 복합 인증 (MFA) [필수] | ID/PW + TOTP (RFC 6238) 복합 인증 | FIA_UAU.5 |
...
| 관리 기능 (Admin Console) | 사용자/클라이언트/역할 관리, 감사 로그 조회, 시스템 설정 | FAU_SAR.1, FMT_SMF.1 |
...
| 1.1 | 2026-03-17 | AuthFusion Team | FIA_UAU.5 추가, MFA/키관리 필수 명시, Admin Console TOE 포함, HMAC-SHA1 추가 |
