...
- **평가 대상**: AuthFusion SSO Server v1.0, AuthFusion Admin Console v1.0, AuthFusion SSO Agent v1.0
...
│  │  │ keyhub             │  │ connect            │  │ automation       │  │  │
│  │  │ (키 관리 허브)     │  │ (연동 커넥터)      │  │ (자동화 워크플로)│  │  │
│  │  └───────────────────┘  └───────────────────┘  └──────────────────┘  │  │
│  │  ┌──────────────────────────────────────────────────────────────────┐  │  │
│  │  │ @ExtendedFeature 확장 컨트롤러                                  │  │  │
│  │  │ - ConsentPageController, OpenApiConfig                          │  │  │
...
| `com.authfusion.sso.jwt` | **TOE** | JWT 토큰 생성/검증, RSA 키 관리 - **필수 (CC 평가 필수 포함)** |
...
| `com.authfusion.sso.mfa` | **TOE** | TOTP MFA, 복구 코드 - **필수 (CC 평가 필수 포함)** |
| `com.authfusion.sso.config.SecurityConfig` | **TOE** | Spring Security 필터 체인 |
| `com.authfusion.sso.ldap` | **EXT** | LDAP 연동 (외부 시스템 의존, 비-TOE) |
...
| TOTP 다중 인증 (RFC 6238) **[필수]** | FIA_UAU.5, FIA_UAU.1, FCS_COP.1 | TotpService, MfaSessionService, MfaController |
...
| RSA 키 생성 **[필수]** | FCS_CKM.1 | KeyPairManager |
| 키 분배 (JWKS) **[필수]** | FCS_CKM.2 | JwkProvider, JwksEndpoint |
| 키 파기 **[필수]** | FCS_CKM.4 | KeyPairManager (rotateKeyPair) |
| 키 암호화 보관 (AES-256-GCM) **[필수]** | FCS_COP.1 | KeyEncryptionService |
...
### 3.3 Admin Console (products/admin-console) - TOE

Admin Console은 TOE의 관리 보안 기능을 제공하는 구성요소로, CC 평가 대상에 포함된다.
관리자는 Admin Console을 통해 사용자/클라이언트/역할 관리, 감사 로그 조회, 시스템 설정 등 보안 관리 기능을 수행한다.

| 항목 | 내용 |
|------|------|
| **분류** | **TOE** (관리 보안 기능 UI) |
| **기술 스택** | Next.js 14, React 18, TypeScript |
| **역할** | SSO Server API를 통한 관리 보안 기능 제공 (FAU_SAR.1, FMT_SMF.1) |
| **CC 모드 동작** | CC 모드에서도 배포 (관리 보안 기능은 평가 대상) |
| **TOE 포함 사유** | 사용자 관리, 감사 로그 조회, 정책 관리 등 관리자 보안 기능을 제공하며, 이는 CC 평가에서 요구하는 관리 기능에 해당 |

#### 3.3.1 Admin Console TOE 보안 기능

| 보안 기능 | 관련 SFR | 구현 페이지/컴포넌트 |
|-----------|---------|---------------------|
| 감사 로그 조회 | FAU_SAR.1 | 감사 로그 페이지 (`/audit`) |
| 사용자 관리 | FMT_SMF.1 | 사용자 관리 페이지 (`/users`) |
| 클라이언트 관리 | FMT_SMF.1 | 클라이언트 관리 페이지 (`/clients`) |
| 역할 관리 | FDP_ACC.1 | 역할 관리 페이지 (`/roles`) |
| 세션 모니터링 | FTA_SSL.3 | 세션 관리 페이지 (`/sessions`) |
| 관리자 인증 | FIA_UAU.1 | 로그인 페이지 (`/login`) |
| 시스템 설정 | FMT_SMF.1 | 설정 페이지 (`/settings`) |

> **MFA(TOTP)와 키 관리는 TOE 필수 구성요소이며, Extension으로 분리할 수 없다.**
...
#### ~~4.1.1 Admin Console~~ → TOE로 재분류 (섹션 3.3 참조)

> **변경 이력**: Admin Console은 관리 보안 기능(사용자 관리, 감사 로그 조회, 정책 관리)을 제공하므로 TOE에 포함한다. CC 모드에서도 배포되어야 한다.
...
- `admin-console` 서비스 배포 (관리 콘솔은 TOE 포함, CC 모드에서도 활성화)
...
| FIA_UAU.5 | 복합 인증 메커니즘 **[필수]** | TotpService, MfaSessionService, MfaController (ID/PW + TOTP 필수) |
...
| 1.1 | 2026-03-17 | Admin Console TOE 포함, MFA/키관리 필수 명시, LDAP 비-TOE 명시, FIA_UAU.5 추가 | AuthFusion Team |
