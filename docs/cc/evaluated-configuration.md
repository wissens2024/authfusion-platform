...
| `SPRING_DATASOURCE_URL` | PostgreSQL JDBC URL | `jdbc:postgresql://db:5432/authfusion_sso` | 기밀 |
| `SPRING_DATASOURCE_USERNAME` | DB 사용자명 | `authfusion` | 기밀 |
| `SPRING_DATASOURCE_PASSWORD` | DB 비밀번호 | (강력한 비밀번호) | **기밀** |
| `AUTHFUSION_SSO_ISSUER` | OIDC Issuer URL | `https://sso.example.com` | 일반 |
...
export SPRING_DATASOURCE_URL=jdbc:postgresql://db-host:5432/authfusion_sso
export SPRING_DATASOURCE_USERNAME=authfusion
export SPRING_DATASOURCE_PASSWORD=$(cat /run/secrets/db-password)
export AUTHFUSION_SSO_ISSUER=https://sso.example.com
...
| 검증 항목 | 확인 방법 | 기대 결과 |
|-----------|----------|----------|
| CC 프로파일 활성화 | `curl localhost:8080/actuator/health` | 200 OK (상세 없음) |
| 확장 기능 비활성화 | `curl localhost:8080/api/v1/clients` | 403 Forbidden |
| Swagger 비활성화 | `curl localhost:8080/swagger-ui.html` | 403 Forbidden |
| OIDC Discovery | `curl localhost:8080/.well-known/openid-configuration` | 200 OK (JSON) |
| 토큰 엔드포인트 | `curl -X POST localhost:8080/oauth2/token` | 400 Bad Request (정상) |
...
| Actuator info | `curl localhost:8080/actuator/info` | 404 Not Found |
